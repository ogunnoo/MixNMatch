    ...
    def train_Dnet(self, idx):
        ...
        if idx == 0:
            ...
            for i in range( self.batch_size ):
                ...
                a2 = min(torch.tensor(self.n_out - 1).float().to(device), torch.floor((self.n_out - 1) - ((62 - self.recp_field) - x2)/self.patch_stride)) + 1
                b1 = max(torch.tensor(0).float().to(device), torch.ceil( (y1 - self.recp_field)/self.patch_stride))
                b2 = min(torch.tensor(self.n_out - 1).float().to(device), torch.floor((self.n_out - 1) - ((62 - self.recp_field) - y2)/self.patch_stride)) + 1
...
if __name__ == "__main__":
    ...
    shutil.copy2( 'model_train_64.py', output_dir)
